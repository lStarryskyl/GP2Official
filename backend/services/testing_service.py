    ...
    if any(kw in t for kw in ["login", "sign in", "authenticate", "credential"]):
        samples = [
            {"type": "positive", "title": f"Successful login with valid credentials",
             ...
             "steps": [f"Enter email: 'sarah.test@example.com'", "Enter password: 'Str0ng!Pass'", "Click Login", "Verify redirect to dashboard"],
             "expected_result": "User is logged in and redirected to the dashboard.",
             "input_label": "email / password", "input_value": "sarah.test@example.com / Str0ng!Pass", "pass_fail": "pass", "reason": "Valid registered credentials", "priority": "high"},
            {"type": "negative", "title": f"Login with unregistered email",
             ...
             "steps": ["Enter email: 'nobody@nowhere.com'", "Enter password: 'Test1234!'", "Click Login", "Verify error message"],
             "expected_result": "System shows error: 'Invalid email or password.'",
             "input_label": "email", "input_value": "nobody@nowhere.com", "pass_fail": "fail", "reason": "Email not registered in the system", "priority": "high"},
            {"type": "negative", "title": f"Login with wrong password",
             "description": f"Test login with correct email but incorrect password.",
             "steps": ["Enter email: 'sarah.test@example.com'", "Enter password: 'wrongpass'", "Click Login", "Verify error message"],
             "expected_result": "System shows error: 'Invalid email or password.'",
    ...
    # --- Registration / Sign Up ---
        ...
        samples = [
            {"type": "positive", "title": f"Successful registration with valid data",
             ...
             "steps": [f"Enter name: '{_rand_name()}'", f"Enter email: '{_rand_email()}'", "Enter password: 'SecureP@ss1'", "Click Register", "Verify account created"],
            ...
            {"type": "negative", "title": f"Registration with already-used email",
             ...
             "steps": ["Enter email: 'existing.user@example.com'", "Fill other fields normally", "Click Register", "Verify duplicate email error"],
             "expected_result": "System shows error: 'This email is already registered.'",
             "input_label": "email", "input_value": "existing.user@example.com", "pass_fail": "fail", "reason": "Email already exists in the system", "priority": "high"},
    ...
    # --- Email ---
        ...
        samples = [
            {"type": "positive", "title": f"Valid email address accepted",
             ...
             "steps": [f"Enter email: 'user.name@company.com'", "Submit", "Verify accepted"],
             "expected_result": "System accepts the email.",
             "input_label": "email", "input_value": "user.name@company.com", "pass_fail": "pass", "reason": "Standard valid email format", "priority": "high"},
            ...
            {"type": "negative", "title": f"Email with spaces",
             ...
             "steps": ["Enter email: 'user name@company.com'", "Submit", "Verify error"],
             "expected_result": "System rejects email with spaces.",
             "input_label": "email", "input_value": "user name@company.com", "pass_fail": "fail", "reason": "Contains spaces — not valid in email", "priority": "medium"},
